...
using System.Data;
public partial class Poll_AnswerPollQuestion : System.Web.UI.Page
{
    private int PollId
    {
        get
    ...
    protected void btnSavePollAnswer_OnClick(object sender, EventArgs e)
    {
        try
        {
            ...
            for (int i = 0; i < DataViewPollQuestion.Items.Count; i++)
            {
                ...
                {
                    dr["TableId"] = Convert.ToInt32(Utility.DecryptQS(Request.QueryString["TId"])); 
                }
                if (Request.QueryString["TT"] != null && Request.QueryString["TT"].ToString() != "")
                {
                    dr["TableType"] = Convert.ToInt32(Utility.DecryptQS(Request.QueryString["TT"]));
                }
    ...
    private void LoadPollInfo()
    {
        ...
        lblPollDescription.Text = PollPollManager[0]["Description"].ToString();
        ...
        {
            HyperLinkAttachment.Visible = false;
        }
