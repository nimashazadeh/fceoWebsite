...
public partial class Members_Documents_WizardQualificationJobConfirm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            Session["DocQualificationJobFileURL"] = null;
            Session["DocQualificationJobGrdURL"] = null;
            SetMenueImage();
            SetHelpAddress();
        ...
        {
            dtJobConfirm = (DataTable)Session["WizardQualificationJobConfirm"];
            GrdvJobCon.DataSource = dtJobConfirm;
            GrdvJobCon.DataBind();
        }

        if (Session["DocQualificationJobFileURL"] != null)
        {
            ImageConf.ImageUrl = Session["DocQualificationJobFileURL"].ToString();
        }
        if (Session["DocQualificationJobGrdURL"] != null)
        {
            ImageGrd.ImageUrl = Session["DocQualificationJobGrdURL"].ToString();
        }
    ...
    protected void GrdvJobCon_RowDeleting(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
    {
        ...
        Session["WizardQualificationJobConfirm"] = dtJobConfirm;
        GrdvJobCon.DataSource = (DataTable)Session["WizardQualificationJobConfirm"];
        GrdvJobCon.DataBind();
        dtJobConfirm = (DataTable)Session["WizardQualificationJobConfirm"];
    ...
    protected void btnNext_Click(object sender, EventArgs e)
    {
        ...
        {
            for (int i = 0; i < ((DataTable)Session["WizardQualificationJobConfirm"]).Rows.Count; i++)
            {
                if (int.Parse(((DataTable)Session["WizardQualificationJobConfirm"]).Rows[i]["ConfirmTypeId"].ToString()) == (int)TSP.DataManager.DocumentJobConfirmType.TwoMembers)
                    towMember++;
            }
    ...
    private void SetMenueImage()
    {
        ...
        if ( Session["WizardQualificationImgfrontDoc"] != null)//|| (Session["WizardQualificationImgTaxOfficeLetter"] != null || CbIAgree()))
        {
    ...
    protected string SaveImageAttach(UploadedFile uploadedFile, string id)
    {
        ...
        if (uploadedFile.IsValid)
        {
            ...
            if (id == "flpConfAttach")
            {
                Session["DocQualificationJobFileURL"] = "~/Image/DocMeFile/JobConfirm/" + ret;
                tempFileName = MapPath("~/Image/DocMeFile/JobConfirm/") + ret;
            }
            if (id == "flpGrdAttach")
            {
                Session["DocQualificationJobGrdURL"] = "~/Image/DocMeFile/OfficeGrade/" + ret;
    ...
    private Boolean InsertJobConfirm()
    {
        ...
        try
        {
            if (Convert.ToInt32(cmbConfirmType.Value) == (int)TSP.DataManager.DocumentJobConfirmType.Office)
            {
                ...
                if (Session["DocQualificationJobFileURL"] != null)
                {
                    dr["FileURL"] = Session["DocQualificationJobFileURL"].ToString();
                    Session.Remove("DocQualificationJobFileURL");
                    Session["DocQualificationJobFileURL"] = null;
                }
                if (Session["DocQualificationJobGrdURL"] != null)
                {
                    dr["GradeURL"] = Session["DocQualificationJobGrdURL"].ToString();
                    Session.Remove("DocQualificationJobGrdURL");
                    Session["DocQualificationJobGrdURL"] = null;
                }
