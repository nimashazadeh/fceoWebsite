...
var slideShow=function(){
	var bxs,bxe,fxs,fxe,ys,ye,ta,ia,ie,st,ss,ft,fs,xp,yp,ci,t,tar,tarl,addressImg;
	...
	return{
		init:function(){
			document.onmousemove=this.pos; window.onresize=function(){setTimeout("slideShow.lim()",5000)};
			ys=this.toppos(ta); ye=ys+ta.offsetHeight;
			len=t.length;tar=[];addressImg=[];
			for(i=0;i<len;i++){
				var id=t[i].value; tar[i]=id; addressImg[i]=t[i].text;
		...
		ImageChanged(id);
			...
			if(!document.getElementById(id)){
				...
				var address=hiddenImageInfo.Get('Images').split(';');
			var p=1;
				for(j=0;j<t.length;j++)
				 if(t[j].value==id)
				 p=address[j];//t[j].text;
				i.style.filter='alpha(opacity=0)';
			//	document.write(id+" "+p);
				i.src=imgdir+'/'+p+imgext;
			}else{
