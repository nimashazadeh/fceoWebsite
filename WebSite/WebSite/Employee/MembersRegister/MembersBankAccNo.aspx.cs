...
using System.Data;
public partial class Employee_MembersRegister_MembersBankAccNo : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            TSP.DataManager.Permission Per = TSP.DataManager.MemberManager.GetUserPermissionForChangeBankAccNo(Utility.GetCurrentUser_UserId(), (TSP.DataManager.UserType)Utility.GetCurrentUser_LoginType());
    ...
    protected void btnNew_Click(object sender, EventArgs e)
    {
        ...
        {
            ShowMessage("به دلیل وجود درخواست بررسی نشده در واحد عضویت امکان تغییر شماره حساب برای این عضو وجود ندارد.");
            return;
        }

        string GridFilterString = GridViewMember.FilterExpression;
        Response.Redirect("MembersBankAccNoInsert.aspx?MeId=" + Utility.EncryptQS(MeId.ToString()) + "&PageMode=" + Utility.EncryptQS("Edit")
    ...
    protected void OnClick_btnSearch(object sender, EventArgs e)
    {
        ...
        if (!Utility.IsCurrentUserFromMainAgentId())
            ...
            ObjectDataSourceMember.SelectParameters["AgentId"].DefaultValue = AgentId.ToString();
