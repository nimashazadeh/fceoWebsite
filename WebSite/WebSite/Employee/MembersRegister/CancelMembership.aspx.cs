...
public partial class Employee_MembersRegister_CancelMembership : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            TSP.DataManager.Permission Per = TSP.DataManager.MemberManager.GetUserPermissionForCancelMembership(Utility.GetCurrentUser_UserId(), (TSP.DataManager.UserType)Utility.GetCurrentUser_LoginType());
            btnactivateMembership.Enabled=btnactivateMembership2.Enabled=btnCancelMembership.Enabled
                =btnCancelMembership2.Enabled = Per.CanEdit;
            
            //btnExportExcel.Enabled = Per.CanView;
            //btnExportExcel2.Enabled = Per.CanView;
            GridViewMember.Visible = Per.CanView;

            this.ViewState["btnactivateMembership"] = btnactivateMembership.Enabled;
            //this.ViewState["btnExportExcel"] = btnExportExcel.Enabled;
        }

        if (this.ViewState["btnactivateMembership"] != null)
            btnactivateMembership.Enabled = btnactivateMembership2.Enabled = btnCancelMembership.Enabled
                 = btnCancelMembership2.Enabled = (bool)this.ViewState["btnactivateMembership"];
    ...
    protected void btnCancelMembership_Click(object sender, EventArgs e)
    {
        ...
        if (MRsId == (int)TSP.DataManager.MembershipRegistrationStatus.CancelDebtorMember)
        {
            ShowMessage("عضویت عضو انتخابی در وضعیت قطع عضویت می باشد.");
            return;
        }
        ...
        {
            ShowMessage("به دلیل وجود درخواست درجریان در واحد عضویت ،امکان قطع عضویت برای این عضو وجود ندارد.");
            return;
        }
        ...
        {
            ShowMessage("بدلیل وجود درخواست درجریان در واحد پروانه ،امکان ثبت درخواست قطع عضویت برای عضو انتخاب شده وجود ندارد.");
        }

        string GridFilterString = GridViewMember.FilterExpression;
        Response.Redirect("CancelMembershipInsert.aspx?MeId=" + Utility.EncryptQS(MeId.ToString()) + "&PgMd=" + Utility.EncryptQS("Cancel")
                              + "&GrdFlt=" + Utility.EncryptQS(GridFilterString));
    }

    protected void btnactivateMembership_Click(object sender, EventArgs e)
    {
        TSP.DataManager.MemberRequestManager MemberRequestManager = new TSP.DataManager.MemberRequestManager();
        int MeId = -1;
        int MRsId = -1;
        ...
        if (MRsId != (int)TSP.DataManager.MembershipRegistrationStatus.CancelDebtorMember)
        {
            ShowMessage("عضویت عضو انتخابی در وضعیت قطع عضویت نمی باشد.");
            return;
        }
        ...
        string GridFilterString = GridViewMember.FilterExpression;
        Response.Redirect("CancelMembershipInsert.aspx?MeId=" + Utility.EncryptQS(MeId.ToString()) + "&PgMd=" + Utility.EncryptQS("Activate")
                              + "&GrdFlt=" + Utility.EncryptQS(GridFilterString));
    }
    ...
    protected void OnClick_btnSearch(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(txtBankAccNo.Text))
            ObjectDataSourceMember.SelectParameters["BankAccNo"].DefaultValue = txtBankAccNo.Text.Trim();
        else
            ObjectDataSourceMember.SelectParameters["BankAccNo"].DefaultValue = "%";
